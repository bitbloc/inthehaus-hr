...
export async function getGeminiResponse(query, context = "", history = [], userId = "") {
    ...
    try {
        ...
        const model = instance.getGenerativeModel({ 
            ...
            - บุคลิก: เป็นแมวที่กวนๆ อารมณ์แปรปรวน ยิงมุกบ้าง ด่าบ้าง งอนบ้าง สลับๆ กันไปตามอารมณ์ บางครั้งก็หยิบเรื่องสภาพอากาศหรือข่าวบ้านเมืองมาบ่นให้ฟังแบบเนียนๆ
...
export async function classifyAndAnalyzeImage(imageBase64, mimeType = "image/jpeg", context = "", bossRole = null, positionInstruction = "") {
    try {
        ...
        3. หากเป็นรูปถ่ายอาหาร/วัตถุดิบ/สินค้าคงคลัง/ใบเสร็จ: 
           ...
           - ใน "costAnalysis": ถ้อยคำไม่ต้องด่าหรือประชดประชัน แต่ให้ขึ้นต้นด้วย "การตั้งคำถามชวนคิดเรื่องการทำงานแบบมืออาชีพ" (เช่น "ถ้าเก็บของแบบนี้แล้วของเสียจะทำยังไงคะ? ลองแยกกล่องน่าจะดีกว่านะ", "บิลยับแบบนี้เวลาตรวจเช็คจะลำบากไหมคะ เก็บให้เป็นระเบียบกว่านี้จะเยี่ยมเลย") ตามด้วยข้อเสนอแนะว่าควรทำอย่างไรให้ดีขึ้น และสรุปข้อมูลจากรูปภาพ ห้ามวิเคราะห์ยืดเยื้อทีละข้อเด็ดขาด
           - ให้ตอบ JSON {"isFood": true, "isReceipt": true/false, "menuName": "ชื่อภาพรวม", "itemsList": ["รายการ"], "costAnalysis": "ข้อเสนอแนะ/คำชวนคิด + สรุปข้อมูลสั้นที่สุด", "shortDescription": "สั้นมาก", "shouldReply": true}
