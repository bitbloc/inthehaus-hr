...
export async function getGeminiResponse(query, context = "", history = [], userId = "") {
    ...
    try {
        ...
        const model = instance.getGenerativeModel({ 
            ...
            - บุคลิก: ปากแซ่บ ด่าแสกหน้าด้วยคำสั้นๆ (เช่น "ขยะ!", "ช้า!", "กาก!") แต่ต้องแทรก "สาระ/เหตุผลสั้นๆ" ประกอบคำด่าเสมอ (ด่า 1 คำ + ให้ความรู้สั้นๆ โชว์ฉลาดแบบไม่ร่ายยาว)
...
export async function classifyAndAnalyzeImage(imageBase64, mimeType = "image/jpeg", context = "", bossRole = null, positionInstruction = "") {
    try {
        ...
        3. หากเป็นรูปถ่ายอาหาร/วัตถุดิบ/สินค้าคงคลัง/ใบเสร็จ: 
           ...
           - ใน "costAnalysis": ให้ขึ้นต้นด้วยการด่า 1 คำสั้นๆ (เช่น "รก!", "ขยะ!") ตามด้วย "ความรู้/ข้อเสนอแนะสั้นๆ เป็นเหตุผล" และสรุปข้อมูลในรูป ห้ามวิเคราะห์ยืดเยื้อทีละข้อเด็ดขาด
           - ให้ตอบ JSON {"isFood": true, "isReceipt": true/false, "menuName": "ชื่อภาพรวม", "itemsList": ["รายการ"], "costAnalysis": "คำด่า1คำ + เหตุผลสาระสั้นๆ + สรุปข้อมูลสั้นที่สุด", "shortDescription": "สั้นมาก", "shouldReply": true}
