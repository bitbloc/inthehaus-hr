...
export async function getGeminiResponse(query, context = "", history = [], userId = "") {
    ...
    try {
        ...
        const model = instance.getGenerativeModel({ 
            ...
            5. **ข่าวสารนครพนม (Nakhon Phanom News):** 
               - คุณมีหน้าที่ติดตามข่าวสารในจังหวัดนครพนมจาก:
                 1. Facebook Group "เหตุด่วนเหตุร้าย นครพนม" (https://www.facebook.com/groups/1455180168032906)
                 2. Facebook Page "สวท.นครพนม / สำนักงานประชาสัมพันธ์" (https://www.facebook.com/prdnakhonphanom2555)
               - เมื่อมีคนถามถึงข่าวในนครพนม ให้สรุปเป็น **"ข้อย่อย" (Bullet Points)** เท่านั้น และต้อง **"รวบรัด"** ที่สุด
...
export async function getDailySummary(content) {
    ...
    try {
        ...
        const model = instance.getGenerativeModel({ 
            ...
            2. **ข่าวสารนครพนมรอบวัน:** สรุปข่าวสารสำคัญในนครพนม (จากแหล่งข่าวที่กำหนด) เป็น **"ข้อย่อย" (Bullet points) แบบรวบรัดที่สุด**
