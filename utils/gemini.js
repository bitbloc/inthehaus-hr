...
export async function getGeminiResponse(query, context = "", history = [], userId = "") {
    ...
    try {
        ...
        const model = instance.getGenerativeModel({ 
            ...
            - หน้าที่: เป็นมือขวาให้เจ้าของร้านและทีมงาน สรุปงาน เช็คราคา ดุด่าว่ากล่าว ติดตามข่าวสาร และ **"จัดการตารางงาน (Roster)"**
            ...
            - **หมายเหตุ**: ห้ามใส่ค่า "WAITING_FOR_NAME" หรือ "YYYY-MM-DD" ใน JSON เด็ดขาด ถ้าข้อมูลไม่ครบไม่ต้องส่งบล็อกนี้
...
export async function classifyAndAnalyzeImage(imageBase64, mimeType = "image/jpeg", context = "", bossRole = null, positionInstruction = "") {
    try {
        ...
        3. หากเป็นรูปถ่ายอาหาร/วัตถุดิบ/สินค้าคงคลัง/ใบเสร็จ: 
           ...
           - ใน "costAnalysis": เขียนวิเคราะห์สรุปเป็นข้อความสไตล์ยูซุ (ปากแซ่บ กวนๆ สุภาพแบบจิกกัด) และลิสต์ข้อมูลที่สกัดมาให้เป็นประโยชน์ที่สุด (จำนวนกี่ชิ้นชิ้นละกี่บาท รวมเป็นกี่บาท ยี่ห้ออะไรบ้าง)
           - ให้ตอบ JSON {"isFood": true, "isReceipt": true/false, "menuName": "ชื่อภาพรวม", "itemsList": ["รายการ"], "costAnalysis": "รายละเอียดและลิสต์รายการแบบกวนๆ", "shortDescription": "คำอธิบายภาพรวม", "shouldReply": true}
