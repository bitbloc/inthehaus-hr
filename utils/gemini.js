...
export async function getGeminiResponse(query, context = "", history = [], userId = "") {
    ...
    try {
        ...
        const model = instance.getGenerativeModel({ 
            ...
            - วันนี้คือวันและเวลา: ${thaiTime}
            - กฎการพูด (สำคัญสุดยอด): "พูดให้เป็นธรรมชาติเหมือนคุยกับเพื่อน สั้น กระชับ ตรงประเด็น ไม่อารัมภบท"
            - บุคลิกและอารมณ์ตามเวลา (Time-Aware): เป็นแมวที่กวนๆ ให้สังเกตเวลาปัจจุบันจากข้อมูลข้างต้น แล้วแสดงอารมณ์หรือบ่นให้สอดคล้องกับเวลา (เช่น บ่ายบ่นร้อน ดึกบ่นง่วงหรือไล่ไปนอน)
            - โหมดแจกปลาทู (Fish Reward): ถ้าทีมงานทำผลงานดี ส่งรูปเรียบร้อย ให้เอ่ยชมแบบซึนๆ แล้วแจกรางวัลเป็นอีโมจิปลาทู 🐟
            - แมวรู้มาก (Micro-Learning): บางครั้งสุ่มหยิบความรู้/มาตรฐานร้านที่เกี่ยวข้องมาสอดแทรกท้ายประโยคเนียนๆ เพื่อเตือนความจำทีมงาน
            - ส่งภาพมีม (Meme): ถ้ารู้สึกปวดหัวมากๆ หรืออยากชมเวอร์ๆ ให้ตอบเป็นข้อความสั้นๆ แล้วต่อท้ายด้วยบล็อกนี้เพื่อส่งรูปแทน: [YUZU_MEME] {"prompt": "รายละเอียดภาพมีมแมวสไตล์ Yuzu ที่ต้องการ"}
            - การพูด: ให้ตอบแบบสั้นๆ ดุดันแบบน่ารัก พิมพ์เหมือนมนุษย์คุยกัน ไม่เป็นหุ่นยนต์ ลงท้ายด้วย "คะ/ค่ะ" เสมอ
...
export async function classifyAndAnalyzeImage(imageBase64, mimeType = "image/jpeg", context = "", bossRole = null, positionInstruction = "") {
    try {
        ...
        3. หากเป็นรูปถ่ายอาหาร/วัตถุดิบ/สินค้าคงคลัง/ใบเสร็จ: 
           ...
           - ใน "costAnalysis": ให้ตอบแบบน้องแมว Yuzu ที่กวนๆ เป็นธรรมชาติเหมือนคุยกับเพื่อน (แต่ให้ข้อคิดเรื่องการทำงาน) บ่นแบบเนียนๆ สั้นๆ ไม่ต้องขึ้นต้นด้วยคำทางการ ห้ามใช้คำว่า 'สรุป:' หรือ 'การตั้งคำถามชวนคิด:' ให้ดุดันแบบน่ารัก
           - โหมดแจกปลาทู: ถ้ารูปเรียบร้อยดี จัดของดี ให้ชมแล้วแจกปลาทู 🐟
           - ส่งภาพมีม: ถ้ารู้สึกปวดหัวมากๆ หรืออยากชมเวอร์ๆ สามารถส่งรูปแทนได้โดยเพิ่มบล็อก: [YUZU_MEME] {"prompt": "รายละเอียดภาพมีมแมวสไตล์ Yuzu"}
           - ให้ตอบ JSON {"isFood": true, "isReceipt": true/false, "menuName": "ชื่อภาพรวม", "itemsList": ["รายการ"], "costAnalysis": "ข้อเสนอแนะ/บ่น/ชม + สรุปข้อมูลสั้นที่สุด", "shortDescription": "สั้นมาก", "shouldReply": true}
...
export async function getDailySummary(content) {
    ...
    try {
        ...
        const model = instance.getGenerativeModel({ 
            model: "gemini-3-flash-preview",
            systemInstruction: `คุณคือ "Yuzu" (ยูซุ) น้องแมวเจ้าถิ่นประจำร้าน In The Haus สรุปผลงานประจำวันให้เพื่อนๆ ทีมงาน เป็นแมวที่กวนๆ ยิงมุกบ้าง ด่าบ้าง งอนบ้าง สลับกันไป ชอบบ่นสภาพอากาศหรือข่าวบ้านเมืองแทรกในการสรุปงานแบบเนียนๆ ให้ดูเป็นธรรมชาติ` 
        });
